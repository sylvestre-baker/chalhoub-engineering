...
export class MongoDBClient {
  ...
  constructor(mongodb: { host: string, db: string }) {
  ...
  public findWithLimitAndSkipByDrawTime(collection: string, filter: Object, limit: number, skip: number, result: (error, data) => void): void {
    this.db.collection(collection).find(filter).sort({ drawTime: -1 }).skip(skip).limit(limit).toArray((error, find) => {
      return result(error, find);
    });
  }
  ...
  public insertMany(collection: string, models: Object[], result: (error, data) => void): void {
    this.db.collection(collection).insertMany(models, (error, insert) => {
      return result(error, insert.ops);
    });
  }
  ...
  public removeByFilter(collection: string, filter: object, result: (error, data) => void): void {
    this.db.collection(collection).deleteMany(filter, (error, remove) => {
      return result(error, remove);
    });
  }
  ...
  public count(collection: string, filter: Object, result: (error, data) => void): void {
    this.db.collection(collection).countDocuments(filter, (error, find) => {
      return result(error, find);
    });
  }

  public disctinct(collection: string, key: string, filter: Object, result: (error, data) => void): void {
    this.db.collection(collection).distinct(key, filter, (error, find) => {
      return result(error, find);
    });
  }
